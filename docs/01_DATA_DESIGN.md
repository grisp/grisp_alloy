...
├── smelterl/              # Local Smelterl checkout; see [Smelterl Design](02_SMELTERL_DESIGN.md)
...
│   └── make_buildroot.sh  # Buildroot make wrapper (console prefix/filter + timestamped br.log)
...
| `ALLOY_DEBUG` | Orchestrator | All hooks | Integer Alloy debug verbosity level (default: `0`). Controls structured Alloy/hook logging only: `0` = errors and warnings only; `1` = add informational progress (`log_info`); `2+` = add developer debug messages (`log_debug`). Set by `--debug` (implies level 1) or `--debug=N`. Hooks use this to gate calls to `log_info` and `log_debug`. See [Alloy Design - §4.9](03_ALLOY_DESIGN.md#49-logging-and-debugging). |
| `ALLOY_BUILDROOT_DEBUG` | Orchestrator | Buildroot wrapper + hooks | Integer Buildroot wrapper verbosity level used by SDK Buildroot invocations: `0` = reduced progress-only console output, `1` = full Buildroot console output, `2+` = full output plus `V=1`. Set by `alloy build sdk -D/-DD` and consumed by `scripts/buildroot/make_buildroot.sh`. |
| `ALLOY_LOG_PREFIX` | Orchestrator/hook entry points | Orchestrator + hooks | Optional structured log prefix consumed by `debug_utils.sh` and `debug_tools.sh`. Typical values: `alloy` (orchestrator), `alloy:<hook_type>` (Buildroot hook wrapper), `alloy:<hook_type>:<nugget>` (hook scripts via `hook_common.sh`). |
